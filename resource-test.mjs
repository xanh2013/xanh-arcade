...
const t=[...m.resource.pending.values()][0];mgr.command(u,'resource-result',{matchId:m.id,taskId:t.taskId,result:{paths:[{id:t.botIds[0],path:Array.from({length:100},(_,i)=>({x:i*100,y:100}))}]}});assert.equal(m.world.resourceOffload.paths.get(t.botIds[0]).length,64);
