...
export function createAdminDonor(api,onState){
 let capabilities=null,running=false,generation=0,timer=null,rtt=0,first=true,usage={cpuMs:0,bytes:0,tasks:0};
 ...
 async function poll(g){if(!running||g!==generation)return;const sample=usage;usage={cpuMs:0,bytes:0,tasks:0};const start=performance.now();
  try{const result=await api('shooter/admin-resource-donor',{enabled:true,start:first,capabilities,rtt,usage:sample});rtt=performance.now()-start;if(!running||g!==generation)return;first=false;
 ...
 return {start(){if(running)return;if(typeof Worker==='undefined'){onState('Trình duyệt chưa hỗ trợ Worker.',false);return;}capabilities=detectResourceCapabilities();running=true;first=true;generation++;onState('Đang kết nối CPU máy này…',true);poll(generation);},stop};
}
